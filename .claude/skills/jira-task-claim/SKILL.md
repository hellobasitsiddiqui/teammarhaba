...
## Loop
1. **Find ready.** JQL: `project = TM AND issuetype = Task AND status = "To Do" AND assignee is EMPTY AND labels != "human" AND sprint in openSprints()`, requesting `fields: ["summary","issuelinks","labels","customfield_10016"]`. Keep only candidates where every inward *Blocks* ("is blocked by") link points to a `Done` issue. The `status = "To Do"` filter already excludes anything **In Progress / In Review / Done** — and since opening a PR flips the ticket to In Review, an **open-PR ticket is never returned** here. (The only way a To-Do ticket has an open PR is a missed `In Review` transition; the step-3 re-verify catches that.)
...
3. **Claim (race-safe — all agents share one Jira user, so status is the lock, not assignee).** `transitionJiraIssue(In Progress)` (this hides it from other agents' `status = "To Do"` query) → `editJiraIssue(assignee = me)` → `addComment("[claim] <agentId> <ISO-time>")` — **keep the returned comment id.** Then **re-verify before starting:** re-read the ticket and **abandon it** (don't touch it, don't roll back status; pick the next) if any holds — (a) status is no longer your `In Progress`; (b) another run left a `PR: <url>` comment (already being worked); (c) the **earliest** `[claim]` is **not the comment you just posted**. ⚠️ Check (c) by **comment identity, not agentId**: two instances sharing an agentId both pass an agentId-only check and both build the ticket — the TM-107 double-build (PR #78 merged, #81 a duplicate). Every running instance also needs a **unique** agentId (`agent-C1`, not two `agent-C`). You get your `agentId` from your kickoff prompt.
4. **Read the blackboard, then work + report.** *First* read `docs/agents/runtime/blackboard.md` (and your `docs/agents/runtime/inbox/<agentId>.md` if present) for anything relevant — env quirks, workarounds, "main red", warnings — so you don't rediscover what another agent already learned. *Then* read the task's **pinned `AGENT EXECUTION PROMPT` comment**, do exactly that. **Branch name: `<type>/TM-XX-short-kebab-desc`** — `<type>` = `feature` (app code), `chore` (infra/CI/cloud/docs/config), or `fix` (bug); e.g. `feature/TM-49-walking-skeleton`, `chore/TM-63-cloud-sql`. **Pre-PR re-verify (closes the claim→build→PR window):** *immediately before you open the PR* (ideally before the final push), re-read the ticket and **abort the PR — keep your branch, post a one-line note, pick the next task** — if any holds: (a) status is no longer your `In Progress` (someone reached In Review/Done first); (b) another run left a `PR: <url>` comment; (c) the earliest `[claim]` is not the comment you posted (identity, not agentId — see step 3). These are the same three checks as the claim-time re-verify, **re-run after the long build**: a duplicate that selected this task *before* your claim landed builds for minutes in its own worktree and never re-reads Jira, so the claim-time check can't catch it — only this pre-PR read can (the TM-151 double-build: PR #145 merged, #146 a duplicate opened ~14 min after selecting). Open a PR. **The moment the PR exists:** `transitionJiraIssue(In Review, id 31)`, add a ticket comment `PR: <url>`, AND return the link in your final message. If the task has **no PR** (a console/settings change), post a one-line evidence note instead and skip straight to Done on completion.
5. **Merged → Testing.** When the PR merges to `main`, `transitionJiraIssue(Testing, id 2)` (the QA gate; `Done`, id 51, is set only after Testing passes — TM-703). The **merge** unlocks dependents automatically — their next poll sees the blocker merged (dependents unlock on merge, not on Done).
...
- **Hit a wall? Log it — never fail silently.** Missing tool / broken env / missing or mis-ordered prerequisite / ambiguous AC → (a) comment the blocker on the ticket **plus** a `[finding → future improvement]` note on how the ticket or its DAG sequence should change; (b) if it's a human-only step (interactive auth, console, secrets), raise a new human-only ticket (labelled `human` — the canonical label the find-ready filter excludes) and link it as a blocker — don't bundle it into an agent task; (c) move to another ready task. These comments are what make each **replay** better than the last (the source gets rebuilt from these tickets).
- **Read shared notes after you claim.** Before starting work, read `docs/agents/runtime/blackboard.md` (append-only operational knowledge — env quirks, workarounds, "main red") and your `docs/agents/runtime/inbox/<agentId>.md` if present. Append cross-cutting findings to the blackboard so no one rediscovers them — but anything that must survive a replay also goes in the ticket or `REPLAY.md` (the blackboard is per-run scratch).
- **Board fields / time tracking** (keep the board functional). On **claim**: set Start date (`customfield_10015` = today) **and** Due date (`duedate` = sprint end). On **PR / In Review**: log a worklog of actual elapsed (`addWorklogToJiraIssue`, `timeSpent` like `"12m"` + `started` = your claim time). If **blocked/held**: set Flagged = Impediment (`customfield_10021` = `[{"value":"Impediment"}]`); clear it when unblocked. Story points are the estimate. Details: `jira-mcp-gotchas` → Time tracking.

...
- **Idle with budget left? Pull a chore:** if the claim loop finds **no ready ticket** and you still have token budget before the current usage window resets, take a low-risk item from `docs/agents/housekeeping.md` (claim it lightly, one small `chore` PR). A ready Jira ticket always wins over housekeeping.

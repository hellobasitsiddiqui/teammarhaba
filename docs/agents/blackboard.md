...
- Public access is **live**: `allUsers` → `roles/run.invoker` on `teammarhaba-backend`; unauthenticated `GET /health` → `200 {"status":"UP"}`. `deploy.yml` now uses `--allow-unauthenticated` + an unauthenticated `/health` curl gate (chore/TM-96 PR).

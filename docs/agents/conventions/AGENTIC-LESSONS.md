...
- **Scope evidence: ONE theme + Android-mobile by default; the full matrix only on explicit request.** Don't dump every surface × every theme — it buries the signal in near-duplicates (302 shots that all share the same app-shell header read as "all the same login screen" even when the content differs). Default the lane to the **mobile surface + the deployed theme + the *named* step-shots** (drop the per-test `test-finished-*` auto-captures and the report's hash-named copies). All-surfaces/all-themes is opt-in — `e2e.yml` `full_evidence=true`, or when the user explicitly asks for "full end-to-end, all themes, all surfaces". And **spot-check that a sample of the shots actually render the target states**, not merely that N files attached.

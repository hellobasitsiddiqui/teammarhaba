...
- **Split every sprint test ticket: automate the deterministic, keep humans for the residue.** Deterministic checks (form CRUD + persistence, validation, role routing, lifecycle, file size/type) go to Playwright e2e against the emulators — with screenshots captured and posted back to the test ticket. Humans only do what automation can't: real email delivery, real prod auth/storage, "does it feel right", post-deploy smoke. Re-testing deterministic flows by hand each sprint is waste and drifts (TM-190 human → TM-195 automated). See `SPRINTS.md` → "Per-sprint testing — always SPLIT".

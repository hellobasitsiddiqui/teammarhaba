...
| 10 | CSS theme-token contract | **Mandatory** | Style only via `var(--token)` and reuse `.tm-*` component classes; **never hard-code hex/px/rgba/font** — it won't swap across clean/doodle/sketch or dark | `web/src/assets/styles.css` | TM-210, TM-211, TM-323 |
| 11 | `data-theme` boot + registry | **Mandatory** | Don't read/apply theme yourself; inherit `data-theme`; if you must, read `document.documentElement.dataset.theme` / `window.TeamMarhabaTheme`; never invent a theme name | `web/src/assets/theme.js` | TM-210, TM-216, TM-323 |

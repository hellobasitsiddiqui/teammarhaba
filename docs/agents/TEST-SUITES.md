...
| `android` | Maestro on an Android emulator (API 34, Pixel 6 profile), debug APK, via [`android/maestro/ci-run.sh`](../../android/maestro/README.md) | **Per-tag Maestro selection is not wired**: whatever `suite` you pass, the job runs the whole shared flow set (`login-sms`, `warm-restart`, `camera`, `biometric`, `permissions`; `login-email` disabled) with a `::notice::` — still meaningful as a smoke over the surface. Each flow starts from clean state (`pm clear`) with the `tm_e2e_phone_test` reCAPTCHA-bypass flag re-injected over CDP; the flag-reading web code must be **deployed** to the hosted SPA for flows to pass. |

...
1. **Automated / agent e2e** (`test` / `e2e`) — the deterministic, repeatable checks: form CRUD + persistence, validation, role routing, lifecycle, account-state, file size/type rules → the **same flows** through CI (web Playwright specs in `web/e2e/` + mobile Maestro), run by the `E2E` workflow against the Firebase emulators. **The FULL set of per-step screenshots is attached as individual viewable images on this automated ticket — not one shot, not just the CI `.zip`; verify the count looks complete (a single screenshot is a red flag).** Playwright shots upload from `test-results/`; **Maestro shots must be harvested from `~/.maestro/tests/…` into the uploaded artifact by the lane (`ci-run.sh`) or they're lost.** The orchestrator/agent runs it.

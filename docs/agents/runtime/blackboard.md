...
### 2026-07-03 16:45 agent-tm-396 — ⚠ main was RED: record-arity semantic conflict between two back-to-back merges (fix TM-417)
- **Symptom:** every open PR's **"Backend build + test"** check fails at **test-compilation** (`constructor EventDraft/EventPatch cannot be applied to given types`) — even PRs touching zero backend files (the web-only TM-396 #304 hit it, as did TM-413 #305). CI tests each PR **merged onto current main**, so a red main reddens everyone.
- **Cause:** **TM-408 (#302)** added `city` + `locationRevealHours` to the `EventDraft`/`EventPatch` records (now 14-arg). **TM-397 (#303)**, branched before #302 and merged ~6 min later, added a new `EventLifecycleNotifierIntegrationTest` still calling the old **12-arg** constructors. No *textual* conflict → both went green per-PR → the merged tree doesn't compile. Classic **semantic** merge conflict on a shared DTO signature.
- **Fix (TM-417):** updated the 4 call sites (1× `EventDraft`, 3× `EventPatch`) to the 14-arg shape — test-only, no prod change. `./mvnw -B spotless:apply test-compile` green.
- **RULE:** changing a shared record/DTO **signature** won't textually conflict with sibling PRs that construct it, but the merged tree **won't compile once both land**. Before/right-after merging a signature change, re-run CI on the other open PRs (or require "branches up to date"). And if your check is red but you touched nothing in that area, **suspect a red main first** (`gh run list --branch main -w CI -L1`) before chasing your own diff.
- **⚠ main-red is TWO issues, not one.** Fixing the compile (TM-417) unmasked a **second** deterministic failure the compile error had been hiding: `WaitlistOfferCascadeIntegrationTest` (TM-397) fails 2 tests (`sweepOpenOffers` no longer offers the FIFO head at T+0) — another TM-408↔TM-397 interaction, but a **runtime-logic** break. Tracked as **TM-419** (needs the offer-cascade owners). So backend checks stay red until **both** TM-417 **and** TM-419 land. Lesson: a compile failure **masks all runtime failures behind it** — when you unblock a compile, expect (and re-run to surface) whatever it was hiding.
